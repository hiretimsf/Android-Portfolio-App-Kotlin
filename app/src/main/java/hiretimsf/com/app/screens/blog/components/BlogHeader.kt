package hiretimsf.com.app.screens.blog.components

import androidx.compose.foundation.layout.Column
import androidx.compose.foundation.layout.fillMaxWidth
import androidx.compose.foundation.layout.padding
import androidx.compose.material3.Text
import androidx.compose.runtime.Composable
import androidx.compose.ui.Modifier
import androidx.compose.ui.res.colorResource
import androidx.compose.ui.text.font.Font
import androidx.compose.ui.text.font.FontFamily
import androidx.compose.ui.text.font.FontWeight
import androidx.compose.ui.text.style.TextOverflow
import androidx.compose.ui.unit.dp
import androidx.compose.ui.unit.sp
import hiretimsf.com.app.R

private val blogHeaderFontFamily = FontFamily(Font(R.font.questrial))

@Composable
fun BlogHeader(modifier: Modifier = Modifier) {
    Column(modifier = modifier.fillMaxWidth()) {
        Text(
            text = "Blog",
            color = colorResource(R.color.colorOnPrimarySurface),
            fontFamily = blogHeaderFontFamily,
            fontWeight = FontWeight.Bold,
            fontSize = 24.sp,
            maxLines = 1,
            overflow = TextOverflow.Ellipsis,
        )
        Text(
            text = "Posts from hiretimsf.com",
            color = colorResource(R.color.colorOnSurface),
            fontFamily = blogHeaderFontFamily,
            fontSize = 15.sp,
            lineHeight = 20.sp,
            modifier = Modifier.padding(top = 4.dp),
        )
    }
}
